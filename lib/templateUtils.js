 ...
 *
 */
var mergeObj = function(template, delta) {
    var result = {};
 ...
 * @param {caf.specType | null} delta Description with changes to apply to template or null if we just want to clone the template.
 *
 * @return {caf.specType} A patched description.
 *
 */
exports.merge = function(template, delta) {
    // TODO: check inputs and the merged output.
    delta = delta || {name : template.name};
    return  mergeObj(template, delta);
};

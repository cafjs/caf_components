...
exports.constructor = function($, spec) {
    ...
    var moduleIndex = {}; // Successfully loaded artefacts (name->module)
    ...
    var load = function(artifact) {
        var result = staticArtifacts[artifact];
     ...
     *  needed.
     * @param {specType=} spec An optional description to be merged-in with
     * the result.
    ...
    that.__ca_loadDescription__ = function(fileName, resolve, spec) {
        ...
        if (resolve) {
            ...
            // 1. Merge template.
            var result = templateUtils.merge(base, delta, false); //clones base
        ...
        try {
            ...
            async.waterfall([
                function(cb1) {
                    ...
                    var p = comp.newInstance(comp$, compSpec, cbOnce);

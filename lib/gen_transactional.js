...
exports.constructor = function($, spec) {
    ...
    var childrenSeriesTransF = function(f, cb) {
        var findTrans = function() {
            ...
            childrenSpec.forEach(function(x) {
                                     if (that.$[x.name].isTransactional) {
                                         result[x.name] = x;
                                     }
                                 });
            return result;
        ...
        if (!transChildrenObj) {
            transChildrenObj = findTrans();
        }
        var trans = childrenSpec
            .filter(function(x) { return transChildrenObj[x.name];});
        async.mapSeries(trans, f, function(err, res) {
                            if (err) {
                                cb(err, res);
                            } else {
                                var result = {};
                                trans.forEach(function(x, i) {
                                                  result[x.name] = res[i];
                                              });
                                cb(err, result);
                            }
    ...
    that.__ca_resume__ = function(cp, cb) {
        ...
        childrenSeriesTransF(function(x, cb1) {
                                 that.$[x.name].__ca_resume__(cp[x.name], cb1);
                             }, cb0);
    };

...
exports.superior = function(target, methodName) {
    ...
    return function() {
        var wrappedMethod = wrapAsyncFunction(method, target);
 ...
 * For example:
 *
 *     var supHello = myUtils.superiorPromisify(that, 'hello');
 *     that.hello = async function() {
 *        try {
 *           var data = await supHello(); // calls original 'hello'
 *        // do something else
 *        } catch (err) {
 *           ...
 *     }
 ...
 *
 *
 * @param {function(cbType)} f An asynchronous function to be wrapped.
...
exports.wrapWithTimeout = function(f, timeout) {
    if (!timeout) {
        ...
        return function(cb) {
            var cbOnce = callJustOnce(null, cb);

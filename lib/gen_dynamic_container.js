...
exports.constructor = function($, spec) {
     ...
     */
    that.__getChildSpec__ = function(name) {
        return childrenSpecObj[name];
    };

    that.__createChild__ = function(data, childSpec, cb) {
        childrenSpecObj[childSpec.name] = childSpec;
        cntUtils.createChild(data, true)(childSpec, cb);
    };

    that.__deleteChild__ = function(data, childName, cb) {
        if (childrenSpecObj[childSpec.name]) {

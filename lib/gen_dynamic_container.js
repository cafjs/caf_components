...
exports.constructor = function($, spec) {
     ...
     *
     */
    var createChild = function(data, childSpec, cb) {
     ...
     * @param {Object=} data An optional hint on how to add the child.
     * @param {Object} childSpec A child description.
     * @param {caf.cb} cb A callback to return an error or the new (or existing)
     * child.
     *
     * @memberof! module:caf_components/gen_dynamic_container#
     ...
     * @param {caf.cb} cb A callback to return an error if a child still exists
     * and `delete` failed.
     *
     * @memberof! module:caf_components/gen_dynamic_container#
    ...
    that.__ca_shutdown__ = function(data, cb) {
        super__ca_shutdown__(data, function(err) {
            if (err) {
                ...
                cntUtils.many('shutdownChild',
                              unknown().concat(known()),
                              data)(function(err) {
                                  if (err) {
                                      var msgLog = 'Error in child shutdown: ' +
                                              spec.name + ' due to ' +

    ...
    var toObject = function(spec) {
        ...
        spec.forEach(function(x) {
                         if (result[x.name]) {
                             ...
                             err.spec = spec;
                             $._.$.log &&
                                 $._.$.log.error(myUtils.errToPrettyStr(err));
                             throw err;
                         } else {
                             result[x.name] = x;
                         }
        ...
        async.series([
                         ...
                         function (cb1) {
                             var cb2 = function(err, res) {
                                 if (err) {
                                     $._.$.log &&
                                         $._.$.log.trace('Restarting children' +
                                                         ' in ' + spec.name +
                                                         ' due to error ' +
                                                         myUtils
                                                         .errToPrettyStr(err));
                             ...
                             cntUtils.many('checkChild',
                                           childrenNames, data)(cb2);
                         }
                     ], cb);
    };
        ...
        super__ca_shutdown__(data, function(err) {
            if (err) {
                ...
                cntUtils.many('shutdownChild',
                              unknown().concat(known()).reverse(),
                              data)(function(err) {
                                  if (err) {
                                      var msgLog = 'Child shutdown error in ' +
                                              spec.name + ' due to ' +
